...
class customer_merchant_fans_visit_api extends Component_Event_Api {
	...
	public function call(&$options) {
	    ...
	    if(!empty($options['longitude']) && !empty($options['latitude'])) {
	        ...
	        $res = RC_Http::remote_get("https://apis.map.qq.com/ws/geocoder/v1/?location=".$options['latitude'].",".$options['longitude']."&key=".$key);
	        $res = json_decode($res['body'], true);
	        if ($res['status'] == 0 && isset($res['result']['ad_info'])) {
	            $area = $res['result']['ad_info']['province'] . ' '. $res['result']['ad_info']['city'];
	        }
	    } else {
	        $area = RC_Ip::area();
	    }
